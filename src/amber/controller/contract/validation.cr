      ...
      def validate
        {% for name, options in FIELD_OPTIONS %}
          ...
          unless {{property_name}}.nil?
            ...
            {% if options[:eq] %}
              p value == {{options[:eq]}}
              error({{property_name.stringify}}, value, "must be equal to {{options[:eq]}}") unless value == {{options[:eq]}}
            {% end %}
            ...
            {% if options[:length] %}
              error({{property_name.stringify}}, value, "must have size in {{options[:length]}}") unless {{options[:length]}}.includes?(value.size)
            {% end %}
